...
DEFINE CLEANER org.uhug.code.sample.pig.udf.matt.SimpleWordCleaner();
...
-- Count the tokens
----------------------------
counts = FOREACH grouped_tokens GENERATE FLATTEN(CLEANER(group)) as key, COUNT_STAR(tokens) as cnt;
...
-- sanity check the tf
----------------------------
projected_tf = FOREACH tf GENERATE tf;
grouped_tf = GROUP projected_tf ALL;
sum_tf = FOREACH grouped_tf GENERATE  SUM(projected_tf);
dump sum_tf;
